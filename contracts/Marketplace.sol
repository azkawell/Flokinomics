...
contract BLEUFINFTMarketplace is ReentrancyGuard, Ownable {
    ...
    uint256 public marketFeeForBNB = 60;
    uint256 public marketFeeForToken = 30;
    ...
    public
    {
        ...
        BLEUFINFT(nftContract).approve(address(this), _id);
        BLEUFINFT(nftContract).transferFrom(msg.sender, address(this), _id);
        listedMap[_id] = true;
        price[_id] = _price;
    ...
    external
    {
        ...
        BLEUFINFT(nftContract).transferFrom(address(this), msg.sender, _id);
        listedMap[_id] = false;
    ...
    function giveaway(address _to, uint256 _id, uint256 _price, 
        ...
        if (_id == 0) {
            ...
            BLEUFINFT(nftContract).mint(newTokenId, _to, _tokenUri);
            emit Giveaway(msg.sender, _to, newTokenId);
        } else {
            if (listedMap[_id] == false) {
                BLEUFINFT(nftContract).transferFrom(msg.sender, _to, _id);
            } else {
                require(ownerMap[_id] == msg.sender, "sender is not owner");
                BLEUFINFT(nftContract).transferFrom(address(this), _to, _id);
                listedMap[_id] == false;
            }
            ownerMap[_id] = _to;
    ...
    function burn(uint256 _id) external {
        BLEUFINFT(nftContract).burn(_id);
    ...
    function buyNew(uint256 _id, 
        ...
        if (keccak256(abi.encodePacked((paymentType))) == keccak256(abi.encodePacked(("BNB")))) {
            ...
            uint256 _commissionValue = price[newTokenId].mul(marketFeeForBNB).div(1000);
            uint256 _sellerValue = price[newTokenId].sub(_commissionValue);
        //    BLEUFINFT(nftContract).transferFrom(msg.sender, receiver, _sellerValue);
        //    BLEUFINFT(nftContract).transferFrom(msg.sender, adminRecoveryAddress, _commissionValue.div(2));
        //    BLEUFINFT(nftContract).transferFrom(msg.sender, payoutAddressMap[paymentType], _commissionValue.div(2));
            transferBNBOrWBNB(payable(receiver),_sellerValue);
            transferBNBOrWBNB(payable(adminRecoveryAddress),_commissionValue);
            // transferBNBOrWBNB(payable(payoutAddressMap[paymentType]),_commissionValue.mul(4).div(10));
            ...
            // require(IERC20(tokenAddressMap[paymentType]).transferFrom(msg.sender, payoutAddressMap[paymentType], _commissionValue.mul(4).div(10)));        
        }
        
        listedMap[newTokenId] = false;
        ownerMap[newTokenId] = msg.sender;
        BLEUFINFT(nftContract).mint(newTokenId, msg.sender, _tokenUri);
    ...
    function _validate(uint256 _id) internal view {
        ...
        require(msg.sender != BLEUFINFT(nftContract).ownerOf(_id), "Can not buy what you own");
    ...
    function updateListingStatus(uint256 _tokenId, bool shouldBeListed) public returns (bool) {
        require(msg.sender == BLEUFINFT(nftContract).ownerOf(_tokenId), "Error, you are not the owner");
    ...
    function createAuction(
        ...
        if (_isNew) {
            ...
            BLEUFINFT(nftContract).mint(tokenId, msg.sender, _tokenUri);
        }
        ...
        onlyCreatorOrWinner(tokenId)
    {
        ...
        if(winner == address(0)) {
            BLEUFINFT(nftContract).transferFrom(address(this), Creator, tokenId);
            ownerMap[tokenId] = Creator;
        } else {
            BLEUFINFT(nftContract).transferFrom(address(this), winner, tokenId);
            if (keccak256(abi.encodePacked((paymentType))) == keccak256(abi.encodePacked(("BNB")))) {
                uint256 _commissionValue = amount.mul(marketFeeForBNB).div(1000);
                transferBNBOrWBNB(payable(adminRecoveryAddress), _commissionValue);
                // transferBNBOrWBNB(payable(payoutAddressMap[paymentType]), _commissionValue.mul(4).div(10));
                if (Creator == creatorMap[tokenId]) {
                    transferBNBOrWBNB(payable(Creator), amount.sub(_commissionValue));
                } else {
                    uint256 _royaltyValue = amount.mul(royaltyMap[tokenId]).div(100);
                    transferBNBOrWBNB(payable(creatorMap[tokenId]), _royaltyValue);
                    transferBNBOrWBNB(payable(Creator), amount.sub(_royaltyValue).sub(_commissionValue));
                }
            } else {
                ...
                // require(IERC20(tokenAddressMap[paymentType]).transfer(payoutAddressMap[paymentType], _commissionValue.mul(4).div(10)));
                if (Creator == creatorMap[tokenId]) {
    ...
    function recoverNFT(uint256 tokenId) external onlyAdminRecovery {
        BLEUFINFT(nftContract).transferFrom(
    ...
    function setMarketFeeForBNB(uint256 _newMarketFeeForBNB) external onlyOwner {
